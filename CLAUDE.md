...
**Data ownership is household-scoped, not flat.** `households` / `household_members` (`role`: `admin` | `member`) sit between `auth.users` and the data: `cats.household_id` (not null) is the ownership boundary, and `records` inherit it transitively via `cat_id → cats.household_id`. RLS on all four tables (`households`, `household_members`, `cats`, `records`, defined in `supabase/schema.sql`) restricts access to rows in a household the requesting user belongs to. Two `security definer` RPCs exist to bootstrap membership — `create_household(name)` (caller becomes `admin` of a new household) and `join_household_by_code(code)` (caller becomes `member` of the household matching `households.invite_code`) — but neither is called from any UI yet; they exist so a future sign-up flow can wire into them without further schema changes. A household's creator (`households.created_by`) can never be removed, even by another admin. Today there is exactly one household, created via a one-off manual SQL backfill (not part of `schema.sql`), with the single shared login as its only `admin` — so in practice this scoping is invisible until multi-account sign-up exists.
